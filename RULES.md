...
- **Aprovação manual do admin (override / "assumir responsabilidade")** — o admin pode aprovar a documentação de uma loja **mesmo sem os 3 documentos enviados/corretos**, assumindo a responsabilidade. Implementado **sem criar caminho de escrita paralelo**: a flag `business_users.documentationAdminApproved` (+ `documentationAdminApprovedAt`) é a **única** escrita nova; toda a derivação continua na fonte única. `syncDocumentationState` passa a usar `effectiveApproved = allApproved || documentationAdminApproved` em **todos** os pontos onde antes usava `allApproved` (resolução de status, escape da expiração, `verified` e visibilidade) — ou seja, a loja com override vira `approved`/`verified=true`/`isVisible=true` e **escapa do `expired`**, herdando exatamente os direitos de quem tem os 3 docs aprovados. `allApproved` (3 docs reais) é preservado **apenas** para a lógica de e-mail em `documents.ts` (não disparar `cadastroAprovado` sem aprovação real de doc). Endpoint: `PATCH /api/admin/documents/business/:businessId/override` body `{approved:boolean}` (grava a flag → `syncDocumentationState(id, { reopenOnApproval:approved })` → envia e-mail no approve → `logAdminAction` `document.manual_approve`/`manual_revoke`). **Reversível**: revogar (`approved:false`) limpa a flag e a fonte única **re-deriva do estado real dos docs** — se a loja estiver fora do prazo sem os 3 aprovados, volta a `expired`/offline. No painel admin a loja recebe o badge roxo "Aprovada manualmente".
  - **Aprovação manual EM MASSA (grandfathering)** — `POST /api/admin/documents/override-all-unapproved` aplica o override (mesma semântica do individual: flag + `syncDocumentationState(reopenOnApproval:true)` + e-mail + audit `document.manual_approve` com `bulk:true`) a **todas** as lojas que ainda **não têm os 3 documentos reais aprovados** (derivado de `business_documents`, não do status agregado) **e** ainda sem override (`documentation_admin_approved=false`). Inclui lojas `expired` (traz de volta ao ar). **Idempotente** (rodar de novo aplica 0; não toca quem tem os 3 docs nem quem já tem override). Botão "Aprovar todas manualmente" no topo de `/admin/documentacao`. **Produção**: o agente NÃO escreve no DB de produção — esse backfill roda **dentro do app** (o dono aciona o botão no painel admin de produção). Reversível loja a loja pelo override individual.
...
- **Defesa em profundidade nos reads públicos (Task #77)**: as listagens públicas (`GET /api/businesses`, `GET /api/search`, `GET /api/zones/:zone/{stats,businesses}`) excluem lojas com `documentationStatus='expired'` via a condição `NOT_DOCUMENTATION_EXPIRED` (subquery correlacionada em `lib/documentation-state.ts`), **além** do filtro de `isVisible`. Como o invariante "`expired`=offline para TODOS os planos" depende de UM único caminho de escrita gravar `isVisible=false`, essa é a **segunda camada**: se algum caminho futuro esquecer o gate, a loja expirada ainda assim **não reaparece** no site público. **NÃO substitui** o filtro de `isVisible` — soma-se a ele.
...
- **Expiração (cron)**: quando `documentationRemainingDays` chega a 0 sem os 3 aprovados, o cron **zera o banco de dias e delega a transição à fonte única `syncDocumentationState`** (Task #77 — não duplica mais a escrita de `expired`+`isVisible=false`), que resolve `documentationStatus='expired'` **e `businesses.isVisible=false` para QUALQUER plano** (gratuito, base/destaque, premium); o cron então dispara `emails.documentacaoExpirada(nome)` apenas quando o estado realmente virou `expired`. O envio dos docs sozinho **não** republica — só a aprovação do admin.
- Emails de contagem regressiva são enviados **a cada 2 dias**, nos dias **pares restantes** `{8, 6, 4, 2}` (não 1/dia e não nos marcos antigos `{7,3,1}`).
