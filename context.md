...
- **Vitrine de Produtos** (`vitrine_boosts`): 4 slots fixos no carrossel de vídeos da home, R$ 49/mês, exclusivo Premium com ≥1 vídeo de produto aprovado. **Compra self-service** pelo próprio lojista em `/lojista/boost` (não depende de admin) — endpoints `GET /api/lojista/vitrine-boost/status`, `POST .../checkout` (subscription) e `POST .../sync`; ativação idempotente também no webhook `checkout.session.completed` (R10/R11). Esgotado (4/4) → a UI desabilita o botão "esgotado este mês"; **mas há fila**: um pagamento que feche com as 4 vagas cheias entra como `waitlist` (sync/webhook decidem `active` se `occupied < 4`, senão `waitlist`). `status.mySlot` reflete a vaga do próprio lojista em qualquer estado aberto (`active`/`waitlist`/`pending`); `used`/`available` contam só `active`. Reserva de 1 linha aberta por lojista via partial unique → `BOOST_ALREADY_EXISTS` em clique duplo (não há gate de lotação no checkout). O vídeo do produto ainda passa por aprovação admin em `/admin/vitrine` antes de contar como elegível.
...
- **Destaque Home + Busca** (`boostContext=home_search`): **3 posições numeradas** (1ª R$249 · 2ª R$179 · 3ª R$129) via `/lojista/boosts/home-search-checkout`. Aparece no topo da home E em todos os resultados de busca. Exclusivo Premium. Cobrança mensal recorrente. Price IDs por posição: `STRIPE_HOME_SEARCH_POS_{1,2,3}_PRICE_ID`. (`HOME_SEARCH_SLOTS=6` em `boost-locks.ts` é contador legado — o modelo vigente é por posição, ver RULES R15.)
- **Destaque de Zona** (`boostContext=zone`): destaque na página da zona por 30 dias (R$79). Máximo **4 vagas** por zona. Quando esgotado, botão fica desabilitado (sem fila de espera). Requer plano Destaque+. Price ID: `STRIPE_ZONE_BOOST_PRICE_ID`. Os compradores aparecem na seção "Destaque para você" na página `/zona/:slug` — **não** em `/busca`. **A seção reserva SEMPRE as 4 vagas** (`zona.tsx`): pagos com selo "Patrocinado" + nas vagas não vendidas um **card-fantasma** (`ZoneAdSlot`) que espelha o layout do `BusinessCard` — banner tingido na cor da zona, logo-fantasma "Seu logo aqui", placeholders de nome/região/estrelas + badge "Esta vaga pode ser sua!" e botão "Anunciar agora" → `/anuncie`. O ranking orgânico dos mais bem avaliados foi removido como bloco separado — segue na grade "Todos os negócios" (ordenada plano→nota→cliques). Só apresentação no front: não toca capacidade/locks/preço/checkout.
- **Banner na Home** (`home_banners`): R$299 **compra única** (`mode:payment`, `price_data` inline — NÃO é assinatura), ativo **30 dias a contar do pagamento** e depois **expira** (sem renovação; para continuar, compra de novo). Máx. 2 lojistas simultâneos. Exclusivo Premium. Sem fila de aprovação desde a Task #56 — após pagamento o lojista faz upload da imagem e o Sharp processa (1200×280) e ativa automaticamente. Gate de plano (`PLAN_REQUIRED`) é checado ANTES do estado do negócio (`BUSINESS_INACTIVE`) em `POST /api/lojista/home-banner/checkout`. As dimensões aceitas (1200×280 · 4:1 · JPG/PNG/WebP · máx 10 MB) aparecem ANTES da compra (Task #64, fonte única `BANNER_SPECS_LINE`). **Upsell de vaga vazia** (`landing.tsx`): quando há **< 2** banners pagos na rotação, a arte de venda padrão ("Anuncie aqui" → `/anuncie`, `id:0`) entra como slide extra (0 pagos → só a arte; 1 pago → pago + arte; 2 pagos → sem arte). A tag "Publicidade" não aparece no slide de venda (`current.id !== 0`). Só apresentação no front.
...
5. Admin revisa em `/admin/documentacao`:
   ...
   - **Aprovar manualmente (override / "assumir responsabilidade")** → mesmo SEM os 3 docs, o admin grava `business_users.documentationAdminApproved=true` via `PATCH /api/admin/documents/business/:businessId/override {approved:true}`. `syncDocumentationState` usa `effectiveApproved = allApproved || documentationAdminApproved`, então a loja vira `approved`/`verified=true`/online e herda os direitos de quem tem doc aprovada. Badge roxo "Aprovada manualmente" no painel admin. **Revogar** (`{approved:false}`) limpa a flag e re-deriva do estado real → loja fora do prazo volta a `expired`/offline. `allApproved` (3 docs reais) é preservado só p/ a lógica de e-mail. Ver RULES.md R2.
   - **Aprovar TODAS manualmente (grandfathering)** → `POST /api/admin/documents/override-all-unapproved` aplica o override (mesma semântica do individual) a todas as lojas sem os 3 docs reais aprovados e ainda sem override (inclui `expired`, traz de volta ao ar). **Idempotente**, retorna `{ok,applied,businessIds}`. Botão roxo "Aprovar todas manualmente" no topo de `/admin/documentacao`. Como o agente não escreve no DB de produção, esse backfill roda dentro do app: o dono clica o botão no painel admin de produção após publicar. Ver RULES.md R2.
...
- Decisão de produto (confirmada pelo dono): liberar produtos no Destaque com limite de **10** e Premium com limite de **20** (NÃO mais ilimitado); Free continua bloqueado (0).
- Backend: 5 endpoints atualizados (POST, PATCH /:id, PATCH reorder, DELETE /:id, POST upload product-media) — checagem mudou de `!== "premium"` para `=== "free"`. Limite é **fonte única** em `lib/enforce-product-limits.ts` (`PRODUCT_LIMITS_BY_PLAN = { free: 0, destaque: 10, premium: 20 }`), usada pela criação/reativação (`getProductLimitForPlan`) e pelo downgrade (`enforceProductLimitForBusiness`). Erros retornam `code: "PRODUCT_LIMIT_REACHED"` com `limit` e `currentPlan`.
